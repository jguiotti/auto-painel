...
export function isClassifiedsOAuthDevStubEnabled(
  ...
  const flag = env.CLASSIFIEDS_OAUTH_DEV_STUB?.trim().toLowerCase();
...
export function buildClassifiedsOAuthDevAuthorizePath(provider: ClassifiedsOAuthDevStubProvider): string {
  return `/api/painel/integracoes/oauth/dev/authorize?provider=${provider}`;
}

export function buildClassifiedsOAuthDevCallbackPath(provider: ClassifiedsOAuthDevStubProvider): string {
  return `/api/painel/integracoes/oauth/dev/callback?provider=${provider}`;
}

export function buildClassifiedsOAuthDevStubCode(
  provider: ClassifiedsOAuthDevStubProvider,
  state: string,
): string {
  return `${CLASSIFIEDS_OAUTH_DEV_STUB_CODE_PREFIX}${provider}:${state}`;
}
...
export function isClassifiedsOAuthDevStubCode(code: string): boolean {
  return parseClassifiedsOAuthDevStubCode(code) !== null;
}
