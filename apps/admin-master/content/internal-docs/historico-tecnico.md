...
Esta página é **somente para a equipe AutoPainel**. Use para consultar decisões de arquitetura, contratos de API, migrações relevantes e links para artefatos no repositório.

## Como consultar e atualizar

1. **Consulta:** abra no admin em **Documentação interna → Documentação técnica** (`/painel/documentacao/tecnica`) ou leia este arquivo no repositório.
2. **Não é preciso preencher no painel** — o conteúdo é **estático**, carregado automaticamente do git.
3. **Atualização:** altere `apps/admin-master/content/internal-docs/documentacao-tecnica.md` via PR após refinamento Architect/Backend; referencie tipos em `packages/shared` e docs em `packages/shared/docs/`.
4. **Visão geral rápida:** comece pelo [Guia da equipe](./guia-da-equipe.md).

### Documentação interna no admin (2026-06-17)

| Rota | Arquivo git | Uso |
| --- | --- | --- |
| `/painel/documentacao/guia-da-equipe` | `guia-da-equipe.md` | Mapa da plataforma, runbooks, índice |
| `/painel/documentacao/regras-de-negocio` | `regras-de-negocio.md` | PRDs e regras BZ/CA |
| `/painel/documentacao/tecnica` | `documentacao-tecnica.md` | Contratos técnicos e histórico de implementação |

Loader: `apps/admin-master/src/lib/internal-docs/load-internal-doc.ts` — prioriza filesystem; formulário de edição no painel desativado (`readOnly`).
...
## Marketing site — SEO, redes sociais e analytics (2026-06-17)

| Campo | Valor |
| --- | --- |
| **App** | `apps/marketing-site` — `https://autopainel.com.br` |
| **Redes (footer)** | Facebook `https://www.facebook.com/autopainel/` · Instagram `https://www.instagram.com/auto_painel/` — `src/lib/brand.ts` (`BRAND_SOCIAL_LINKS`) |
| **SEO** | `metadata` root (`layout.tsx`), `sitemap.ts`, `robots.ts`, `opengraph-image.tsx`, JSON-LD `MarketingJsonLd` com `sameAs` |
| **IA / crawlers** | `public/llms.txt` — resumo público para LLM/search agents |
| **GTM** | `AutopainelGoogleTagManager*` com `ap_app_surface=marketing`; consent gate em `cookie-consent-banner` |
| **Eventos** | `pushAutopainelAnalyticsEvent` — v1 `lead_form_submit` em `contact-form.tsx` |
| **Docs operacionais** | `packages/shared/docs/GTM.md`, `GTM_EVENTS.md`; comando `/marketing` actualizado |
| **Sem segredos no git** | `NEXT_PUBLIC_GTM_ID` só em `.env.local` / Vercel — nunca em markdown público |

### Provisionamento loja demo Meta App Review

| Item | Referência |
| --- | --- |
| Migração seed | `20260615200000_seed_meta_review_demo_dealership.sql` |
| Vercel hosts | `npm run dealership:hosts:provision -- demo` |
| DNS | `DEALERSHIP_HOSTS_PROVISIONING.md` |
| Credenciais revisor | `gestor.demo@autopainel.demo` — ver `META_INTEGRATION_SIMPLIFIED.md` §8.6 (operador; não commitar senha) |
...
| **E2-M5** | Instagram carrossel Graph API (≥2 slides) + imagem única (1 slide) | ✅ `social-publish-process-job.ts` |
| **E2-M6** | OAuth Meta UX (popup postMessage, cleanup, page picker) | ✅ `social-meta-integration-card.tsx`, `meta/oauth/cleanup`, `connection-status` |
...
**Fix OLX OAuth `missing_code` (2026-06-10):** redirect URI **com** `?provider=olx`; UI diagnóstico + logs `[AutoPainel OAuth]`. **Fix OLX OAuth `invalid_callback` (2026-06-15):** redirect URI OLX passa a ser **URL base** (sem `?provider=` — a OLX substitui a query no redirect); `state` prefixado `ap:olx:…`; callback Edge redireciona erros para `/api/painel/integracoes/oauth/popup-result`; UI aguarda 2,5s antes de cleanup no `popup_closed`. Migração `20260615180000_olx_oauth_redirect_uri_base.sql`. **Diagnóstico `session_invalid` em localhost:** painel com `NEXT_PUBLIC_SUPABASE_URL=127.0.0.1:54321` grava sessão local mas callback OLX é remoto → `oauth/start` bloqueia com `oauth_session_store_mismatch` (2026-06-15); ver `CLASSIFIEDS_OAUTH_SETUP.md` §0.1. **iCarros (2026-06-12):** doc `api.icarros.com.br/oauth-api` usa **password grant** (não popup Keycloak) — UI integrador pendente; tabela `dealership_classifieds_integrator_accounts` preparada. **WebMotors INT-5b (2026-06-12):** formulário integrador CRM + `POST /api/painel/integracoes/webmotors/connect` + migração `20260612170000_webmotors_integrator_accounts.sql`. **WebMotors INT-5c (2026-06-10):** `webmotors-adapter.ts` + catálogo `/catalogo/v1/salesforce/used` + refresh via integrador; default `WEBMOTORS_LISTINGS_API_URL` = `estoque/v1`; homolog live bloqueada até plano API Site (ex.: Guiotti aguardando comercial). **Santander simulador:** fase 2 — sem alteração em `calculate-finance-simulation.ts`.
...
**Meta finalize (2026-06-10):** popup OAuth sem `noopener` (postMessage); cleanup `POST /api/painel/integracoes/meta/oauth/cleanup`; status `GET …/connection-status`; seletor de página reabre; IG publica imagem única quando render devolve 1 slide; scope `business_management`; smoke `npm run meta:config:smoke`. Go-live: `META_INTEGRATION_SIMPLIFIED.md` §9.
| **INT-1** | Auto-publish pós create/update (P1–P6 **por portal**) | 🟢 entregue | `maybe-auto-publish-classifieds-on-save.ts`, `actions.ts`, opt-out `skip_classifieds_promotion` |
| **INT-2** | Delist antes de `deleteVehicleAction` | 🟢 entregue | `delist-classifieds-before-delete.ts`, `deleteVehicleAction` |
| **INT-3** | Provider `icarros` end-to-end (connect) | ⏸ aguardando credenciais iCarros | `icarros/connect` entregue; homologação comercial pendente |
| **INT-4** | Refresh token + `reauth_required` | 🟢 entregue | OLX: `refresh_token`; WebMotors: password grant integrador — `classifieds-refresh-access-token.ts` |
| **INT-5** | Homologação APIs reais OLX | 🟢 adapter OLX autoupload entregue | `olx-adapter.ts` — ativar com `CLASSIFIEDS_SYNC_DRY_RUN=false` na Edge |
| **INT-5b** | WebMotors connect (integrador CRM) | 🟢 entregue | `webmotors/connect`, `dealership_classifieds_integrator_accounts` |
| **INT-5c** | WebMotors publish/delist + catálogo | 🟢 código entregue | `webmotors-adapter.ts`, `webmotors-catalog-resolve.ts` — homolog live aguarda plano API Site por loja |
| **INT-7** | Republicar após update (preço/fotos) | 🟢 entregue | INT-1 auto-publish no update |
...
| 2 | 🟡 blocker | Publicação social real depende App Review Meta + `SOCIAL_PUBLISH_DRY_RUN=false` (código OAuth/worker entregue) | DevOps + QA manual | open |
...
- **Home whitelabel v1 (2026-06-10):** copy e imagem de hero por loja em `content_config.storefront_home`. Tipos: `packages/shared/src/types/dealership-config.ts` (`StorefrontHomeConfig`, `StorefrontHomeLayoutCopy`). Resolução + defaults: `packages/shared/src/lib/dealership/storefront-home-copy.ts` (`resolveStorefrontHomeCopy`, `mergeStorefrontHomeConfig`, placeholder `{nome_loja}`). Admin: aba Vitrine — `storefront-home-content-fields.tsx` + `dealership-form.tsx`; persistência e upload hero (5 MB, bucket `dealership-branding`) em `apps/admin-master/src/actions/dealerships.ts` (`buildContentConfig`, `resolveHeroBackgroundUrlFromForm`). Vitrine: `home-hero.tsx` (CTAs estoque + WhatsApp), `home-heritage-section.tsx`, `storefront-home-layout.tsx`; footer — `storefront-footer-social-links.tsx` + `parse-dealership-social-links.ts` lê `content_config.social_links`. **Sem migração SQL** — JSON em coluna existente. BZ: `BZ-WH-H*` em `regras-de-negocio.md`.
...
| `gestor.demo@autopainel.demo` | **demo** (Meta App Review) | `LojaDemo123!` |

### Loja demo Meta App Review (produção) — 2026-06-10

| Campo | Valor |
| --- | --- |
| **Migração** | `supabase/migrations/20260615200000_seed_meta_review_demo_dealership.sql` |
| **Slug / nome** | `demo` / **Demo** |
| **Plano** | enterprise · layout 1 · tema dark (indigo) |
| **Conteúdo** | `content_config.storefront_home` + logos placehold.co + 6 veículos `demo-*` |
| **Painel prod.** | `https://demo.loja.autopainel.com.br/painel/integracoes` |
| **Vitrine prod.** | `https://demo.autopainel.com.br` |
| **DNS / Vercel** | `npm run dealership:hosts:provision -- demo` + CNAME Registro.br — `DEALERSHIP_HOSTS_PROVISIONING.md` |
| **Doc operacional** | `packages/shared/docs/META_INTEGRATION_SIMPLIFIED.md` §8.6 |
...
| **Admin master** | `AdminNotificationProvider` — alertas **operacionais da plataforma** (`dealership_billing_history` atraso/vencimento próximo + `dealerships` `pending_setup`/`suspended`/`churned`); **não** consome `leads`; escopo lido `admin-platform-ops`; refresh 90s |
| **Painel lojista** | `DealershipNotificationProvider` — leads **da loja** (`dealership_id`) com `source` vitrine (`vehicle_page`, `finance_simulator`, `contact_page`, `whatsapp_float`); exclui `manual`; Realtime filtrado; escopo lido `dealership-{id}-storefront-leads`; owner/manager/super_admin |
| **Shared mappers** | `lead-notification-items.ts`, `platform-billing-notification-items.ts` |
