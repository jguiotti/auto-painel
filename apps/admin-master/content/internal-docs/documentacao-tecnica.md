...
- **`resolve_dealership_id_by_host_for_dashboard`**: mesma lógica de host **sem** filtro de `status`, usada em caminhos `/painel` e `/login` no `dealership-panel`.
- **`get_dealership_id_by_slug_for_dashboard`**: em `localhost` com `DEVELOPMENT_TENANT_SLUG`, resolve o `id` pelo slug **sem** exigir `active` (paridade com o resolver de host).
...
Secrets Supabase Dashboard (Edge): `META_APP_CLIENT_ID`, `META_APP_CLIENT_SECRET`, `META_TOKENS_CRYPTO_SECRET`, opcionais `META_OAUTH_REDIRECT_URI`, `META_GRAPH_API_VERSION`, `META_OAUTH_SCOPES`. Next (painel) necessita **`META_APP_CLIENT_ID`** para URL de autorização; redirect usa `META_OAUTH_REDIRECT_URI` ou **`NEXT_PUBLIC_SUPABASE_URL`** + `/functions/v1/meta-oauth-callback`. **URLs de OAuth Meta / Graph podem estar em constante revisão oficial — sempre validar com a doc Meta.**
