...
import {
  encryptClassifiedsSecretValue,
} from "@autopainel/shared/lib/classifieds-token-crypto";
import { createSupabaseServiceRoleClient } from "@autopainel/shared/lib/supabase/service-role";
import { NextResponse, type NextRequest } from "next/server";

import { ClassifiedsOAuthNotConfiguredError } from "@/lib/classifieds/oauth-not-configured-error";
import {
  exchangeWebMotorsPasswordGrantToken,
  WebMotorsPasswordGrantError,
} from "@/lib/classifieds/exchange-webmotors-password-grant";
import { classifiedsProviderUnavailableMessage } from "@/lib/integrations/integration-user-messages";
import { resolveWebMotorsPlatformConfig } from "@/lib/classifieds/resolve-webmotors-platform-config";
...
export async function POST(request: NextRequest) {
  ...
  const activeFeatures = Array.isArray(featuresRes.data)
    ...
    : [];
  if (!isClassifiedsProviderModuleEnabled(activeFeatures, "webmotors")) {
    return NextResponse.json(
  ...
  if (!username || !password) {
    return NextResponse.json(
      { error: "Informe o usuário e a senha do integrador CRM WebMotors." },
      { status: 400 },
    );
  }
  ...
  if (isClassifiedsOAuthDevStubEnabled()) {
    const stub = buildClassifiedsOAuthDevStubTokenPayload("webmotors");
    tokenPayload = {
    ...
    let platformConfig;
    try {
      platformConfig = await resolveWebMotorsPlatformConfig();
    } catch (error) {
    ...
    try {
      tokenPayload = await exchangeWebMotorsPasswordGrantToken({
      ...
      const supportCode =
        error instanceof WebMotorsPasswordGrantError ? error.supportCode : "webmotors_auth_failed";
      const message =
        error instanceof WebMotorsPasswordGrantError
          ? error.message
          : "Não foi possível validar o integrador CRM. Tente novamente.";
  ...
  if (integratorError) {
    return NextResponse.json(
      { error: "Conexão criada, mas não foi possível guardar o integrador. Tente reconectar." },
      { status: 500 },
    );
  }
