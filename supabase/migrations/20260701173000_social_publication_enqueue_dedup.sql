-- migration: deduplicated enqueue for social_publication_jobs (post once per vehicle)
...
comment on function public.enqueue_social_publication_job(uuid, text[], text, jsonb, text) is
  'Enqueues a social publication job once per vehicle; rejects duplicate pending jobs and republish after success.';

revoke all on function public.enqueue_social_publication_job(uuid, text[], text, jsonb, text) from public;
grant execute on function public.enqueue_social_publication_job(uuid, text[], text, jsonb, text) to authenticated;
