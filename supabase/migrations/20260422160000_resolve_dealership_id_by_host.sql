/*
  migration: resolve dealership uuid from incoming host (custom domain or platform subdomain)
  security: security definer; filters by custom_domain or slug + platform root domain only
*/
...
comment on function public.resolve_dealership_id_by_host(text, text) is
  'maps request host to dealership id via custom_domain or slug as first label of platform domain';
